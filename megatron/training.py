...
from megatron.model import (
    GPT2ModelPipe,
    # TokenformerModelPipe,
    SoftEmbedding,
    get_params_for_weight_decay_optimization,
    get_params_for_multilr_weight_decay_optimization,
)
...
def get_optimizer(model, neox_args):
    ...
    if neox_args.old_lr_decay:
        # weight_decay_params, no_weight_decay_params, lr_decay_weight_decay_params, lr_decay_no_weight_decay_params
        param_groups = get_params_for_multilr_weight_decay_optimization(model, neox_args)
    else:
...
def setup_model_for_eval(neox_args, use_cache=False, iteration=None):
    ...
    assert neox_args.eval_ckpt is not None, 'can not find any eval ckpts.'
    model.module.load_state_dict(torch.load(neox_args.eval_ckpt))
