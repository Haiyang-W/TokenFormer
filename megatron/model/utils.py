...
def get_params_for_multilr_weight_decay_optimization(module, neox_args):
    """Divide params into with-weight-decay and without-weight-decay groups.
    Layernorms and biases will have no weight decay but the rest will.
    """
    # weight_decay_params = {"params": [], "name": "weight_decay_params"}
    weight_decay_params = {"params": [], "name": "weight_decay_params"}
    lr_decay_weight_decay_params = {"params": [], "name": "lr_decay_weight_decay_params", "lr_decay": neox_args.old_lr_decay_scale,}
    no_weight_decay_params = {
        "params": [],
        "weight_decay": 0.0,
        "name": "no_weight_decay_params",
    }
    lr_decay_no_weight_decay_params = {
        "params": [],
        "weight_decay": 0.0,
        "name": "lr_decay_no_weight_decay_params",
        "lr_decay": neox_args.old_lr_decay_scale,
    }
    for module_ in module.modules():
        if any(
            [
            ...
            #     [p for p in list(module_._parameters.values()) if p is not None]
            # )
            if getattr(module_, "is_new", False):
                no_weight_decay_params["params"].extend(
                    [p for p in list(module_._parameters.values()) if p is not None]
                )
            else:
                lr_decay_no_weight_decay_params["params"].extend(
                    [p for p in list(module_._parameters.values()) if p is not None]
                )
        else:
            if getattr(module_, "is_new", False):
                ...
                lr_decay_weight_decay_params["params"].extend(
                    [
                        p
                        for n, p in list(module_._parameters.items())
                        if p is not None
    ...
    if neox_args.weight_decay == 0.0:
        ...
        # with onebitadam, we want to minimize the calls to compressed_allreduce. Every param group calls it once.
        # to avoid this, only use a single param group when weight decay is off.
        return [no_weight_decay_params, lr_decay_no_weight_decay_params]
    return weight_decay_params, no_weight_decay_params, lr_decay_weight_decay_params, lr_decay_no_weight_decay_params
    ...
    def _is_checkpointable(self, funcs):
        if self.parent_class_name == "GPT2ModelPipe":
            return all(
                ...
                "ParallelTokenformerLayerPipe" in f.__class__.__name__ for f in funcs
            ) or all(
                "ParallelTokenformerChunkLayerPipe" in f.__class__.__name__ for f in funcs
            ) or all(
                "ParallelTransformerChunkLayerPipe" in f.__class__.__name__ for f in funcs
            )
