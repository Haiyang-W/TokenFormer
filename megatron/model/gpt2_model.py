...
from megatron.model.norms import get_norm, get_final_norm
...
from megatron.model.tokenformer import ParallelTokenformerLayerPipe
from megatron.model.gmlp import GMLPBlock
from megatron.model.mamba import ParallelMambaResidualLayerPipe
from megatron.model.word_embeddings import EmbeddingPipe, SoftEmbedding, IncrementalEmbeddingPipe
...
class GPT2ModelPipe(PipelineModule, torch.nn.Module):
    ...
    def init_specs(self):
        ...
        if weight_tying:
            if self.neox_args.transformer_increase_dim > 0:
                self.specs.append(
                    TiedLayerSpec(
                        "embed",
                        IncrementalEmbeddingPipe,
                        self.neox_args,
                        self.hidden_size,
        ...
        for i in range(self.neox_args.num_layers):
            ...
            elif layer_type in ["tokenformer"]:
                self.specs.append(
                    LayerSpec(
                        ParallelTokenformerLayerPipe,
                        neox_args=self.neox_args,
        ...
        if self.neox_args.final_norm is None:
            norm, eps = get_norm(self.neox_args)
        else:
            norm, eps = get_final_norm(self.neox_args)
        if self.neox_args.transformer_increase_dim > 0:
            self.specs.append(
                LayerSpec(NormPipe, norm, self.neox_args.hidden_size+self.neox_args.transformer_increase_dim, eps=eps)
            )
        else:
            self.specs.append(
        ...
        def _logits_helper(embedding, lm_output):
            ...
            if getattr(embedding, "word_embeddings_new", None) is None:
                ...
                # extra embedding
                logits = parallel_lm_logits(
                    lm_output, torch.cat([embedding.word_embeddings_weight, embedding.word_embeddings_new.weight], dim=1), self.parallel_output
                )
            return logits
