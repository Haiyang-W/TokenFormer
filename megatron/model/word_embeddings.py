...
class Embedding(torch.nn.Module):
    ...
    def __init__(
        ...
        # if neox_args.frozen_old:
        #     for pn, p in self.named_parameters():
        #         if pn.endswith("word_embeddings.weight"):
...
class IncrementalEmbedding(torch.nn.Module):
    ...
    def __init__(
        ...
        self.word_embeddings_new = mpu.VocabParallelEmbedding(
            neox_args=neox_args,
            num_embeddings=vocab_size,
            embedding_dim=neox_args.transformer_increase_dim,
            init_method=self.init_method,
        )
        self.word_embeddings_new.is_new = True
        self._word_embeddings_key = "word_embeddings"
    ...
    def forward(self, input_ids, position_ids, tokentype_ids=None):
        # Embeddings.
        words_embeddings = torch.cat([self.word_embeddings(input_ids), self.word_embeddings_new(input_ids)], dim=-1)
        if self.use_pos_emb and self.embedding_type in ["learned", "sinusoidal"]:
            if self.opt_pos_emb_offset:
                if self.layer_past is not None:
                    position_ids = position_ids + self.layer_past + 1
...
class IncrementalEmbeddingPipe(IncrementalEmbedding):
    """Extends Embedding to forward attention_mask through the pipeline."""
