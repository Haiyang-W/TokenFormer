...
class NeoXArgs(*BASE_CLASSES):
    ...
    @classmethod
    def consume_neox_args_eval(cls, overwrite_values=None, input_args=None):
        """
        Deepspeed launcher needs to pass the arguments for `pretrain_gpt2.py` across to all machines.
    ...
    def calculate_derived(self):
        ...
        if self.num_kv_heads is not None:
            ...
            if self.num_kv_heads < self.num_attention_heads:
                ...
                assert all(
                    (attn_type == "flash") or (attn_type == "global") or (attn_type == "tokenformer")
        ...
        if "flash" in self.attention_config or "tokenformer" in self.attention_config:
