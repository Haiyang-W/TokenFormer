...
class HFDataSource(grain.RandomAccessDataSource):
  ...
  def __init__(self, dataset: datasets.IterableDataset, dataloading_host_index: int, dataloading_host_count: int, num_threads: int, train: bool):
  ...
  def _update_shard(self, idx):
    ...
    if self.dataset_shards[idx] >= self.n_shards:
      ...
      self.dataset_shards[idx] -= self.n_shards
