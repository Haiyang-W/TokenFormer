...
class Decoder(nn.Module):
  ...
  def get_decoder_layer(self):
    ...
    elif self.config.decoder_block == "tokenformer":
      from layers import tokenformer

      return tokenformer.TokenFormerDecoderLayer
  ...
  def get_norm_layer(self):
    if self.config.decoder_block in ("default", "llama2", "mistral", "gemma", "simple"):
      return RMSNorm
    elif self.config.decoder_block in ("gpt3", "tokenformer"):
      from layers import gpt3
  ...
  def scan_decoder_layers(self, cfg, decoder_layer, length, metdata_axis_name, mesh):
    # self.scan_decoder_layers(cfg, RemattedBlockLayer, cfg.num_decoder_layers, "layers", mesh)
