...
def train_step(model, config, state, data, dropout_rng, pre_grad_norm):
  """
  ...
  Args:
    ...
    data: Batch of data to apply to the model
    dropout_rng: A key to use to generate rng for dropout
    pre_grad_norm: A scalar used to prevent NaN loss
  ...
  # the follow code is to skip certain steps when grad or grad_norm is NaN
  new_state = jax.lax.cond(jnp.logical_or(jnp.isnan(raw_grad_norm), jnp.isnan(loss)),
                           lambda x, y: x,
                           lambda x, y: x.apply_gradients(grads=y), state, grads)

  grad_norm = jax.lax.cond(jnp.logical_or(jnp.isnan(raw_grad_norm), jnp.isnan(loss)),
                           lambda: pre_grad_norm,
                           lambda: raw_grad_norm)
  
  skip = jax.lax.cond(jnp.logical_or(jnp.isnan(raw_grad_norm), jnp.isnan(loss)),
                           lambda: 1,
                           lambda: 0)
  ...
  metrics = {
      "scalar": {
          ...
          "learning/skip_step": skip,
          "learning/pre_grad_norm": pre_grad_norm,
          "learning/upated_pre_grad_norm": grad_norm,
      },
      "scalars": {},
  }
  ...
  return new_state, metrics, grad_norm
...
def setup_mesh_and_model(config):
  ...
  if config.use_constant_lr:
    learning_rate_schedule = optax.constant_schedule(config.learning_rate)
  else:
...
def setup_train_loop(config):
  ...
  state, state_mesh_annotations, data_iterator, unboxed_abstract_state = max_utils.setup_training_state(
...
def train_loop(config, state=None):
  ...
  max_logging.log(f"number parameters: {num_model_parameters/1e6:.3f} million")
  ...
  if jax.process_index() == 0:
    if config.metrics_file:
      local_metrics_dir = config.metrics_file
    else:
      local_metrics_dir = os.path.join('/home/yfan/logs/', config.run_name)
    if not os.path.exists(local_metrics_dir):
      os.makedirs(local_metrics_dir)
    local_metrics_file = open(os.path.join(local_metrics_dir, 'train_results.txt'), "a", encoding="utf8")
  else:
    local_metrics_file = None
  ...
  def create_skipped_iterator(config, mesh, step):
    # the purpose of this function is to automatically resume data_iterator when the job dies unexpected
    # NOTE: this trick assumes you are using GCP buckets to store the ckpt
    # NOTE: make sure to change the name "maxtext_multihost_job" in get_bucket()
    # NOTE: this trick only works with grain data loader
    while True:
      ...
      try:
        ...
        bucket = storage_client.get_bucket('maxtext_multihost_job')  # TODO: hard code
  ...
  pre_grad_norm = 1000.  # use a large enough init value so that the first train step can start
  for step in np.arange(start_step, config.steps):
    ...
    with jax.profiler.StepTraceAnnotation("train", step_num=step):
      try:
        ...
        data_iterator = create_skipped_iterator(config, mesh, step)
        example_batch = load_next_batch(data_iterator, example_batch, config)
      check_example_batch(config, example_batch=example_batch)
      nextrng = jax.jit(jax.random.fold_in)(init_rng, step)
      record_goodput(recorder, config, step=step)
      with mesh, nn_partitioning.axis_rules(config.logical_axis_rules):
        state, metrics, pre_grad_norm = p_train_step(state, example_batch, nextrng, pre_grad_norm)
    ...
    if (config.eval_interval > 0 and step > start_step and step % config.eval_interval == 0) or (step == config.steps - 1):
      ...
      for eval_batch in eval_data_iterator:
        ...
        if config.dataset_type == 'grain':
          if eval_batch_count * config.global_batch_size_to_load >= len(eval_data_iterator.dataloader._data_source) - config.global_batch_size_to_load:
            break
        with mesh, nn_partitioning.axis_rules(config.logical_axis_rules):
          eval_metrics = p_eval_step(state, eval_batch, nextrng)
        print(">>> eval step: ", eval_batch_count)
      ...
      if jax.process_index() == 0:
        if config.metrics_file:
          local_metrics_dir_eval = config.metrics_file
        else:
          local_metrics_dir_eval = os.path.join('/home/yfan/logs/', config.run_name)
        with open(os.path.join(local_metrics_dir_eval, 'results.txt'), 'a') as f:
          f.write(f'Step: {step}; eval_loss: {eval_loss}\n')
      if eval_loss <= config.target_eval_loss:
        max_logging.log(f"Early stop and exit loop after reaching {config.target_eval_loss=}")
        prof.deactivate()
        break
      print(f">>> evaluation ends with {eval_batch_count} steps!!!")

    if step == last_profiling_step:
      prof.deactivate()
  print(">>> Training finishes!!!")
  if checkpoint_manager is not None:
    checkpoint_manager.wait_until_finished()  # Blocks until the checkpoint saving is completed.
...
def main(argv: Sequence[str]) -> None:
  ...
  print(">>>>> jax process", jax.process_index())
